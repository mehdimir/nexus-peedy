...
{
    public RepositoriesUI()
    {
        setSpacing( true );

        addComponent( new Label( "<h3>Errors in caption</h3>", Label.CONTENT_XHTML ) );
        addComponent( new Label( "Error indicators are usually placed on the right side of the component's caption." ) );

        TextField input = new TextField( "Field caption" );
        input.setComponentError( new UserError( "This field is never satisfied" ) );
        addComponent( input );

        addComponent( new Label( "<h3>Errors without caption</h3>", Label.CONTENT_XHTML ) );
        ...
        addComponent( new Label( "<h3>Error icon placement depends on the layout</h3>", Label.CONTENT_XHTML ) );
        addComponent( new Label(
