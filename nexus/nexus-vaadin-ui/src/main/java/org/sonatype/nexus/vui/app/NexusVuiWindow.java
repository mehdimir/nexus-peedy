...
public class NexusVuiWindow
    extends Window
{
    private final ThemeResource EMPTY_THEME_ICON = new ThemeResource( "../sampler/sampler/icon-empty.png" );

    private final ThemeResource SELECTED_THEME_ICON = new ThemeResource( "../sampler/sampler/select-bullet.png" );
    ...
    // itmill: UA-658457-6
    private final GoogleAnalytics webAnalytics = new GoogleAnalytics( "UA-658457-6", "none" );
    ...
    public NexusVuiWindow( NexusVuiApplication nvu )
    {
        ...
        this.breadcrumbs = new BreadCrumbs( nexusVuiApplication );
        ...
        setCaption( "Nexus SPEEDY" );
        setTheme( nexusVuiApplication.getCurrentApplicationTheme() );
        ...
        ActiveLink allSamples = new ActiveLink( "All Features", new ExternalResource( "#" ) );
    ...
    private Component createThemeSelect()
    {
        ...
        for ( String themeName : NexusVuiApplication.THEMES )
        {
            String id = NexusVuiApplication.SAMPLER_THEME_NAME + "-" + themeName;
            theme.addItem( id );
        ...
        theme.addListener( new ComboBox.ValueChangeListener()
        {
            public void valueChange( ValueChangeEvent event )
            {
                ...
                nexusVuiApplication.setCurrentApplicationTheme( newTheme );
            }
        } );
    ...
    private ModeSwitch createModeSwitch()
    {
        ...
        m.addMode( currentList, "", "View as Icons", new ThemeResource( "../sampler/sampler/grid.png" ) );
        /*- no CoverFlow yet
        m.addMode(coverFlow, "", "View as Icons", new ThemeResource(
                "../sampler/sampler/flow.gif"));
         */
        m.addMode( new FeatureTable( nexusVuiApplication ), "", "View as List", new ThemeResource(
            ...
            for ( Iterator<Feature> it = features.iterator(); it.hasNext(); )
            {
                ...
                else
                {
                    ...
                    if ( rootSet.getParent() == null )
                    {
                        ...
                        ActiveLink sample =
                            new ActiveLink( f.getName(),
                                new ExternalResource( "#" + nexusVuiApplication.getPathFor( f ) ) );
                        sample.setIcon( res );
                        if ( f.getSinceVersion().isNew() )
                        {
